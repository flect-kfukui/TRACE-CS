...
def post_process_explanation(
    ...
    scheduled_course_descriptions = {
        course: desc
        for course, desc in course_descriptions.items()
        if course in scheduled_course_codes
    }

    # Format the schedule and course descriptions
    schedule_str = "\n".join(
        [
            f"Semester {i + 1}: {', '.join(courses)}"
            for i, courses in enumerate(schedule)
        ]
    )
    course_descriptions_str = "\n".join(
        [f"{course}: {desc}" for course, desc in scheduled_course_descriptions.items()]
    )
...
Post-processed explanation:"""
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an AI assistant helping a student understand their course schedule explanations.",
            },
...
def process_query(
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are an AI assistant and your task is to extract information a user queries.",
            },
            {"role": "user", "content": prompt},
        ],
        max_tokens=300,
        ...
        if course_name in scheduler.courses_taken:
            query_data.append(
                (
                    course_name,
                    semester,
                    f"{course_name} has already been taken in a past semester.",
                    "",
                )
            )
                ...
                if int(semester) > 8:
                    query_data.append(
                        (
                            course_name,
                            semester,
                            f"There is no semester {int(semester)}. The maximum amount of semesters is 8.",
                            "",
                        )
                    )
                    ...
                    query_data.append(
                        (
                            course_name,
                            semester,
                            f"Semester {int(semester)} has already passed.",
                            "",
                        )
                    )
                    ...
                    if condition == "positive":
                        ...
                        query_data.append(
                            (
                                ...
                                semester_number,
                                True,
                                scheduler.course_vars[course_name]["semester_vars"][
                                    semester_number
                                ],
                            )
                        )
            ...
            query_data.append(
                (course_name, semester, "This course does not exist.", "")
            )
...
def contrastive_explanations(
    ...
    scheduled_courses = [
        course for semester in schedules[current_schedule_index] for course in semester
    ]
    ...
    for query_course_code, semester_number, condition, query_var in query_data:
        ...
        if condition == "This course does not exist.":
            Explanations.append(
                f"{query_course_code} does not exist in the available course catalog."
            )
            return Explanations
        elif (
            condition == f"Semester {semester_number} has already passed."
            or condition
            == f"There is no semester {semester_number}. The maximum amount of semesters is 8."
        ):
            Explanations.append(condition)
            return Explanations
        elif (
            condition
            == f"{query_course_code} has already been taken in a past semester."
        ):
            Explanations.append(condition)
            return Explanations
            ...
            for semester, courses in enumerate(schedules[current_schedule_index]):
                if query_course_code in courses:
                    var = [
                        scheduler.course_vars[query_course_code]["semester_vars"][
                            semester - scheduler.current_semester
                        ]
                    ]
            ...
            if condition == True:  # Positive condition (course should be taken)
                if semester_number is not None:
                    ...
                    OR_clause = [
                        v
                        for v in scheduler.course_vars[query_course_code][
                            "semester_vars"
                        ]
                    ]
                ...
                not_taken_prereqs = [
                    c
                    for c in scheduler.courses[query_course_code]["prerequisites"]
                    if c not in scheduled_courses and c not in courses_in_query
                ]
                if not_taken_prereqs:
                    Explanations.append(
                        f"{query_course_code} cannot be scheduled because its prerequisite course(s) {', '.join(not_taken_prereqs)} has not been completed."
                    )
        ...
        while True:
            if SAT(KB, updated_true_lits):
                ...
                #     return Explanations
                    ...
                    Explanations.append(
                        "The query does not conflict with the scheduling constraints, and thus it can be satisfied."
                    )
                    return Explanations
...
def explain_why_not_query(
    ...
    for query_course_code, semester_number, condition, query_var in query_data:
        if condition:  # Positive condition (course should be taken)
            ...
            if query_prereqs_incomplete:
                explanations.append(
                    f"{query_course_code} cannot be scheduled in semester {semester_number + scheduler.current_semester + 1} because the prerequisite course(s) {', '.join(query_prereqs_incomplete)} has not been completed."
                )
    ...
    if sol:
        if all_pos_query_courses:
            explanations.append(
                f"Course(s) {', '.join(course[0] for course in all_pos_query_courses)} can be scheduled in semester(s) {', '.join(str(course[1] + scheduler.current_semester + 1) for course in all_pos_query_courses)}."
            )
        elif all_neg_query_courses:
            explanations.append(
                f"Course(s) {', '.join(course[0] for course in all_neg_query_courses)} can be removed from semester(s) {', '.join(str(course[1] + scheduler.current_semester + 1) for course in all_neg_query_courses)}."
            )
    else:
