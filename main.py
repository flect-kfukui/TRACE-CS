...
def update_navigation_buttons() -> None:
    ...
    next_schedule_button.config(
        state=tk.NORMAL if current_schedule_index < len(schedules) - 1 else tk.DISABLED
    )
        ...
        if semester in scheduler.taken_courses:
            for j, course in enumerate(scheduler.taken_courses[semester]):
                course_button = ttk.Button(
                    schedule_frame, text=course, style="Gray.TButton"
                )
                # course_button = tk.Button(schedule_frame, text=course, bg="green", fg="white", relief=tk.RIDGE, borderwidth=2)
                course_button.bind(
                    "<Enter>", lambda event, c=course: display_course_details(event, c)
                )
...
previous_schedule_button = ttk.Button(
    export_frame, text="Previous Schedule", command=previous_schedule
)
previous_schedule_button.pack(side=tk.LEFT, padx=(0, 10))

next_schedule_button = ttk.Button(
    export_frame, text="Next Schedule", command=next_schedule
)
...
query_label = ttk.Label(
    query_frame, text="Please enter your query:", font=("Arial", 12, "bold")
)
    ...
    if not invalid_query:
        if confirm_button is None:
            ...
            confirm_button = ttk.Button(
                confirmation_frame,
                text="Confirm",
                command=lambda: confirm_query(query, query_data),
                style="Red.TButton",
            )
...
def confirm_query(query: str, query_data: List[Any]) -> None:
    ...
    if explanations:
        post_processed_explanations = [
            post_process_explanation(
                explanations,
                query,
                schedules[current_schedule_index],
                scheduler.courses,
            )
        ]
        explanation_text.explanations = (
            explanations  # Store the pre-processed explanations
        )
        explanation_text.insert(tk.END, post_processed_explanations[0])
        current_explanation_index = 0
        ...
        similarity_button = ttk.Button(
            similarity_frame,
            text="Semantic Similarity",
            command=lambda: calculate_semantic_similarity(
                explanations[current_explanation_index], post_processed_explanations
            ),
            style="Blue.TButton",
        )
    ...
    for widget in confirmation_frame.winfo_children():
        if widget.winfo_class() == "TButton" and widget["text"] in [
            "Confirm",
            "Reject",
        ]:
            widget.destroy()
...
def reject_query() -> None:
    ...
    for widget in similarity_frame.winfo_children():
        if (
            widget.winfo_class() == "TButton"
            and widget["text"] == "Semantic Similarity"
        ):
            widget.destroy()
...
submit_button = ttk.Button(
    query_frame, text="Submit Query", command=submit_query, style="Submit.TButton"
)
